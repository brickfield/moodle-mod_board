...
/**
 * Test update_column event.
 *
 * @package    mod_board
 * @copyright  2025 Brickfield Education Labs <https://www.brickfield.ie/>
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 * @covers     \mod_board\event\update_column
 */
final class update_column_test extends \advanced_testcase {
    public function test_event(): void {
        ...
        column::update($column->id, 'Col X');
        $events = $sink->get_events();
        $sink->close();
        $this->assertCount(1, $events);
        $event = $events[0];
        $this->assertInstanceOf(\mod_board\event\update_column::class, $event);
        ...
        $this->assertSame('Column updated', $event->get_name());
        $this->assertSame('Col X', $event->other['name']);
