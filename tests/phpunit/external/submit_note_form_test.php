...
final class submit_note_form_test extends \advanced_testcase {
    public function test_execute(): void {
        ...
        $cm1 = board::coursemodule_for_board($board1);
        ...
        $note = $DB->get_record('board_notes', ['id' => $result['note']['id']]);
        $this->assertSame($columns1[0]->id, $note->columnid);
        $this->assertSame($student1->id, $note->ownerid);
        $this->assertSame($student1->id, $note->userid);
        $this->assertSame(null, $note->groupid);
        $this->assertSame('haha', $note->content);
        $this->assertSame('pokus', $note->heading);
        ...
        $this->assertSame($student1->id, $note->userid);
        $this->assertSame(null, $note->groupid);
        $this->assertSame('hahaX', $note->content);
        $this->assertSame('pokusX', $note->heading);
        $this->assertSame('0', $note->type);
        $this->assertSame(null, $note->info);
        $this->assertSame(null, $note->url);
        $this->assertTimeCurrent($note->timecreated);
